...
import {
  ...
  USER_UPDATE_SUCCESS,
  USER_UPDATE_FALI,
  USER_TPOSELLERS_LIST_REQUEST,
  USER_TPOSELLERS_LIST_SUCCESS,
  USER_TPOSELLERS_LIST_FAIL,
  ...
  try {
    ...
    const message =
      error.response && error.response.data.message
        ? error.response.data.message
        : error.message;
    dispatch({ type: USER_UPDATE_FALI, payload: message });
  }
};

// Primero retormanos una funcion que retorna otra funcion redux-func que recibe dos parametros (dispatch, getState)
// en el body de esta funcion necesito despachar user list request
...
export const listTopSellers = () => async (dispatch) => {
  dispatch({ type: USER_TPOSELLERS_LIST_REQUEST });
  try {
    const { data } = await Axios.get('/api/users/top-sellers');
    dispatch({ type: USER_TPOSELLERS_LIST_SUCCESS, payload: data });
  } catch (error) {
    const message =
      error.response && error.response.data.message
        ? error.response.data.message
        : error.message;
    dispatch({ type: USER_TPOSELLERS_LIST_FAIL, payload: message });
  }
};
